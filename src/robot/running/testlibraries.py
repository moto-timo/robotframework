...
def TestLibrary(name, args=None, variables=None, create_handlers=True):
    ...
    # DEBUG:
    LOGGER.warn("testlibraries.py TestLibrary")
    LOGGER.warn("libclass = '%s'" % libclass)
...
class _BaseTestLibrary(object):
    ...
    def __init__(self, libcode, name, args, source, variables):
        LOGGER.warn("testlibraries.py _BaseTestLibrary __init__")
        ...
        self._libinst = None
        LOGGER.warn("    self.__dict__ = '%s'" % self.__dict__)
    ...
    def _valid_init(self, method):
        return inspect.ismethod(method) or is_java_init(method) or is_dotnet_init(method)
    ...
    def _get_handler_method(self, libcode, name):
        ...
        # DEBUG:
        LOGGER.warn("method = '%s'" % method)
        LOGGER.warn("libcode = '%s'" % libcode)
        LOGGER.warn("name = '%s'" % name)
        if is_dotnet_method(method):
            raise ValueError("dot net method")
        return method
...
class _ClassLibrary(_BaseTestLibrary):
    ...
    def _get_handler_method(self, libinst, name):
        ...
        if IRONPYTHON:
            ...
            try:
                clr_type = clr.GetClrType(libinst)
                ...
                pass
            if isinstance(libinst, clrtype.ClrClass):
                pass
    ...
    def _validate_handler(self, handler):
        '''TODO: what is implicit xxx method being used for?'''
        if not self._is_routine(handler):
            raise DataError('Not a method or function')
        if self._is_implicit_java_or_jython_method(handler): # or self._is_implicit_dotnet_method(handler):
            raise DataError('Implicit methods are ignored')

    def _is_routine(self, handler):
        return inspect.isroutine(handler) or is_java_method(handler) or is_dotnet_method(handler)
    ...
    def _is_implicit_dotnet_method(self, handler):
        if not is_dotnet_method(handler):
            return False
        #for signature in handler.argslist[:handler.nargs]:
        #    cls = signature.declaringClass
        #    if not (cls is Object or self._is_created_by_ironpython(handler, cls)):
        #        return False
        return True

    def _is_created_by_ironpython(self, handler, cls):
        # do something here
        return False #do something else here

    def _get_action(self, lib, method):
        action = Action[lib](method);
        return action;

    def _get_function(self, lib, method):
        function = Func[lib](method);
        return function;
